...
describe('resolveUserDataDir', () => {
  it('설치본이 실제로 쓰는 경로를 그대로 가리킨다 — 실행 중인 v1.0.31에서 실측한 값', () => {
    // 표시 이름을 Braindump로 바꾼 뒤에도 기존 세션·설정이 살아 있어야 한다.
  ...
  it('productName을 따라 새 디렉터리를 만들지 않는다', () => {
    // 'DROP'·'Braindump'는 둘 다 빈 디렉터리다. 여기로 가면 전원 로그아웃이다.
    const resolved = resolveUserDataDir(APP_DATA, true)
    expect(resolved.endsWith('/DROP')).toBe(false)
    expect(resolved).not.toContain('Braindump')
  })
